...
class BSSCRun:
    ...
    def saveSample(self, location):
        self.monitorAsynchronousMethod(self.bssc.recuperate(location.getPlate(), location.getRowAsInt(), location.getColumn()+2))
    
    def loadBuffer(self, location):
        self.monitorAsynchronousMethod(self.bssc.fill(location.getPlate(), location.getRowAsInt(), location.getColumn()+1, 10.0))
    
    def expose(self, duration):
        #taskid = self.bssc.push(self.samplevolume-self.bssc.getBeamMarkVolume(), (self.samplevolume-self.bssc.getBeamMarkVolume())/duration)
        gda.jython.commands.ScannableCommands.staticscan([self.detector, self.cam])
    ...
    def run(self):
        ...
        for titration in self.bean.getMeasurements():
            ...
            self.reportSampleProgress(titration, "Recuperating Sample")
            self.saveSample(titration.getLocation())
